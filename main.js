...
/* ===== Modern Hero Slider (additive, non-destructive) =====
   Enhances the existing .slideshow-container1 in place.
   Does NOT modify or remove any existing code/functions. */
(function(){
    'use strict';
    ...
    function initModernHeroSlider(){
        var container = document.querySelector('.slideshow-container1');
        if(!container || container.classList.contains('modern-hero-slider')) return;
        var slides = Array.prototype.slice.call(container.getElementsByClassName('mySlides1'));
        if(!slides.length) return;

        container.classList.add('modern-hero-slider');

        // 1) horizontal track; move existing slides into it (nothing deleted)
        var track = document.createElement('div');
        track.className = 'mhs-track';
        ...
        for(var s=0;s<slides.length;s++){
            ...
            if(cfg.title || cfg.subtitle || cfg.cta){
                var ov = document.createElement('div');
                ov.className = 'mhs-overlay';
                var html = '';
                if(cfg.title){ html += '<h2 class="mhs-title">'+esc(cfg.title)+'</h2>'; }
                if(cfg.subtitle){ html += '<p class="mhs-sub">'+esc(cfg.subtitle)+'</p>'; }
                if(cfg.cta){ html += '<a class="mhs-cta" href="'+esc(cfg.link||'#products')+'">'+esc(cfg.cta)+'</a>'; }
                ov.innerHTML = html;
                slides[s].appendChild(ov);
            }
        }

        // 3) modern pill indicators with autoplay progress fill
        var pills = document.createElement('div');
        pills.className = 'mhs-pills';
        for(var p=0;p<slides.length;p++){
            (function(idx){
                var btn = document.createElement('button');
                btn.type = 'button';
                btn.className = 'mhs-pill';
                btn.setAttribute('aria-label', 'الانتقال إلى الشريحة ' + (idx+1));
                btn.innerHTML = '<span class="mhs-pill-fill"></span>';
                btn.addEventListener('click', function(){ go(idx, true); });
                pills.appendChild(btn);
            })(p);
        }
        container.appendChild(pills);

        // 4) hide old dots row (reversible: just display:none)
        var oldDot = document.querySelector('.dot1');
        if(oldDot && oldDot.parentElement && !oldDot.parentElement.classList.contains('mhs-pills')){
            oldDot.parentElement.style.display = 'none';
            oldDot.parentElement.setAttribute('data-mhs-hidden','1');
        }

        var index = 0, timer = null;
        var pillEls = pills.getElementsByClassName('mhs-pill');
        var fillEls = pills.getElementsByClassName('mhs-pill-fill');

        function render(){
            track.style.transform = 'translateX(' + (-index * 100) + '%)';
            for(var k=0;k<pillEls.length;k++){
                if(k===index){ pillEls[k].classList.add('active'); }
                else { pillEls[k].classList.remove('active'); }
            }
            for(var f=0;f<fillEls.length;f++){
                fillEls[f].style.transition = 'none';
                fillEls[f].style.width = '0%';
            }
            if(fillEls[index]){
                void fillEls[index].offsetWidth;
                fillEls[index].style.transition = 'width ' + AUTOPLAY_MS + 'ms linear';
                fillEls[index].style.width = '100%';
            }
        }
        ...
        function restartTimer(){ if(timer){ clearInterval(timer); } timer = setInterval(next, AUTOPLAY_MS); }

        // 5) keep existing controls working (arrows + old dots drive the new slider)
        window.plusSlides1 = function(n){ go(index + (n || 0), true); };
        window.currentSlide1 = function(n){ go((n || 1) - 1, true); };

        // 6) swipe on the track (left = next, right = previous)
        var sx=0, sy=0, tracking=false;
